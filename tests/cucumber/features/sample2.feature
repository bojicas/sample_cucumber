Feature: One-liner description of this feature

  As a [role]
  I want [feature]
  So that [benefit]

  The story above is to set context for the reader. It doesn't actually have any impact on the test
  itself. The phrases inside the scenarios are ties to test code using regex, which you can see in
  /tests/features/step_definitions/steps.js

  In this textual part of the file, you can include context about this feature, you can add links to
  external documents and whatever is needed to create the full specification.

  # The background will be run for every scenario
  Background:
    Given I am a new user

  # This scenario will run as part of the Meteor dev cycle because it has the @dev tag
  @dev
  Scenario: Scenario number 1
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 2
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 3
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 4
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 5
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 6
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 7
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 8
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 9
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 10
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 11
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 12
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 13
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 14
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 15
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 16
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 17
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 18
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 19
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
  Scenario: Scenario number 20
    When I navigate to "/"
    Then I should see the title "Sample Cucumber"
